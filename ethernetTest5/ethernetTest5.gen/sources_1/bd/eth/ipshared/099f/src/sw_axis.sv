...
module sw_axis
  #(
    ...
    parameter			      PREFIX_CHARS = 21,
    parameter			      POSTFIX_CHARS = 10,
    // those values
    parameter [(8*PREFIX_CHARS)-1:0]  PREFIX_STRING = "Temperature reading: ",
    parameter [(8*POSTFIX_CHARS)-1:0] POSTFIX_STRING = " Degrees C",
    // GPIO input width
   ...
   // Timer trigger
        ...
        always_ff @(posedge clk) begin
            if (rst) begin
                ...
                send_trigger <= 0;
                if (timer == 5_000_000-1) begin  // 100ms at 50MHz
                    timer <= 0;
                    send_trigger <= 1;
                end else begin
                    timer <= timer + 1;
   ...
   always @(*)
     begin
        ...
        case (current_state)
          IDLE   :
            begin
               if (send_trigger & ~fifo_full & axis_ready) begin
   ...
   generate;
      for (i = 1;i<=GPIO_WIDTH/4;i=i+1) begin
         assign gpio_ascii[i*8-1 -: 8] = gpio_in_debounced[i*4-1 -: 4] > 9 ? gpio_in_debounced[i*4-1 -: 4]+55 : gpio_in_debounced[i*4-1 -: 4]+48;
      end
   endgenerate
